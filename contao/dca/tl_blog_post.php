...
$GLOBALS['TL_DCA']['tl_blog_post'] = array
(
	// Config
	'config' => array
	(
		'dataContainer'               => DC_Table::class,
		'ptable'                      => 'tl_blog_archive',
		...
		(
			array('tl_blog_post', 'checkPermission'),
			array('tl_blog_post', 'generateFeed')
		),
		'oncut_callback' => array
		(
			array('tl_blog_post', 'scheduleUpdate')
		),
		...
		(
			array('tl_blog_post', 'adjustTime'),
			array('tl_blog_post', 'scheduleUpdate')
		),
		'oninvalidate_cache_tags_callback' => array
		(
			array('tl_blog_post', 'addSitemapCacheInvalidationTag'),
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		'pid' => array
		(
			'foreignKey'              => 'tl_blog_archive.title',
		...
		(
			...
			'save_callback' => array
			(
				array('tl_blog_post', 'generateAlias')
			),
		...
		(
			...
			'eval'                    => array('url_callback'=>array('tl_blog_post', 'getSerpUrl'), 'title_tag_callback'=>array('tl_blog_post', 'getTitleTag'), 'titleFields'=>array('pageTitle', 'headline'), 'descriptionFields'=>array('description', 'teaser')),
			'sql'                     => null
		),
		'subheadline' => array
		(
		...
		(
			...
			'filter'                  => true,
			'inputType'               => 'radio',
			'options_callback'        => array('tl_blog_post', 'getSourceOptions'),
			'reference'               => &$GLOBALS['TL_LANG']['tl_blog_post'],
		...
		(
			'exclude'                 => true,
			'inputType'               => 'select',
			'options_callback'        => array('tl_blog_post', 'getArticleAlias'),
...
class tl_blog_post extends Backend
{
	...
	/**
	 * Check permissions to edit table tl_blog
	 *
	...
	public function checkPermission()
	{
		...
		{
			$key = array_search('allowComments', $GLOBALS['TL_DCA']['tl_blog_post']['list']['sorting']['headerFields'] ?? array());
			unset($GLOBALS['TL_DCA']['tl_blog_post']['list']['sorting']['headerFields'][$key], $GLOBALS['TL_DCA']['tl_blog_post']['fields']['noComments']);
		}
		...
		switch (Input::get('act'))
		{
			...
			case 'copy':
				...
				{
					$objArchive = $this->Database->prepare("SELECT pid FROM tl_blog WHERE id=?")
												 ->limit(1)
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		{
			return $this->Database->prepare("SELECT id FROM tl_blog WHERE alias=? AND id!=?")->execute($alias, $dc->id)->numRows > 0;
		};

		// Generate alias if there is none
		if (!$varValue)
		{
			$varValue = System::getContainer()->get('contao.slug')->generate($dc->activeRecord->headline, BlogArchiveModel::findByPk($dc->activeRecord->pid)->jumpTo, $aliasExists);
		}
	 ...
	 * @return string
	 */
	public function getTitleTag(BlogPostModel $model)
	{
		/** @var BlogArchiveModel $archive */
	...
	public function adjustTime(DataContainer $dc)
	{
		...
		$this->Database->prepare("UPDATE tl_blog %s WHERE id=?")->set($arrSet)->execute($dc->id);
	}
	...
	{
		$archiveModel = BlogArchiveModel::findByPk($dc->activeRecord->pid);
