...
$GLOBALS['TL_DCA']['tl_blog_archive'] = array
(
	// Config
	'config' => array
	(
		'dataContainer'               => DC_Table::class,
		'ctable'                      => array('tl_blog_post'),
		'switchToEdit'                => true,
		'enableVersioning'            => true,
		'markAsCopy'                  => 'title',
		'onload_callback' => array
		(
			array('tl_blog_archive', 'checkPermission'),
			array('tl_blog_archive', 'generateFeed')
		),
		'oncreate_callback' => array
		(
			array('tl_blog_archive', 'adjustPermissions')
		),
		...
		(
			array('tl_blog_archive', 'scheduleUpdate')
		),
		'oninvalidate_cache_tags_callback' => array
		(
			array('tl_blog_archive', 'addSitemapCacheInvalidationTag'),
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			(
				'href'                => 'act=edit',
				'icon'                => 'header.svg',
				'button_callback'     => array('tl_blog_archive', 'editHeader')
			),
			'copy' => array
			(
				'href'                => 'act=copy',
				'icon'                => 'copy.svg',
				'button_callback'     => array('tl_blog_archive', 'copyArchive')
			),
			'delete' => array
			(
				'href'                => 'act=delete',
				...
				'button_callback'     => array('tl_blog_archive', 'deleteArchive')
			),
			'show' => array
			(
				'href'                => 'act=show',
	...
	(
		...
		(
			...
			'reference'               => &$GLOBALS['TL_LANG']['tl_blog_archive'],
...
class tl_blog_archive extends Backend
{
	...
	public function checkPermission()
	{
		...
		{
			unset($GLOBALS['TL_DCA']['tl_blog_archive']['fields']['allowComments']);
		}
		...
		if (!$security->isGranted(ContaoNewsPermissions::USER_CAN_CREATE_ARCHIVES))
		{
			...
			$GLOBALS['TL_DCA']['tl_blog_archive']['config']['notCopyable'] = true;
		}
	...
	public function adjustPermissions($insertId)
	{
		...
		if (is_array($arrNew['tl_blog_archive']) && in_array($insertId, $arrNew['tl_blog_archive']))
		{
	...
	{
		return System::getContainer()->get('security.helper')->isGranted(ContaoCorePermissions::USER_CAN_EDIT_FIELDS_OF_TABLE, 'tl_blog_archive') ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.svg$/i', '_.svg', $icon)) . ' ';
	}
