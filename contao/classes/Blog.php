...
/**
 * Provide methods regarding blog archives.
 */
class Blog extends Frontend
{
	...
	{
		$objFeed = BlogFeedModel::findByPk($intId);
		...
		$objFeed->feedName = $objFeed->alias ?: 'blog' . $objFeed->id;
		...
		else
		{
			...
			System::getContainer()->get('monolog.logger.contao.cron')->info('Generated blog feed "' . $objFeed->feedName . '.xml"');
		}
	}
	...
	{
		$objFeed = BlogFeedModel::findByArchive($intId);
	...
	protected function generateFiles($arrFeed)
	{
		...
		{
			$objArticle = BlogPostModel::findPublishedByPids($arrArchives, null, $arrFeed['maxItems']);
		}
		else
		{
			$objArticle = BlogPostModel::findPublishedByPids($arrArchives);
		}
		...
		if ($objArticle !== null)
		{
			...
			while ($objArticle->next())
			{
				...
				{
					$strDescription = '';
					$objElement = ContentModel::findPublishedByPidAndTable($objArticle->id, 'tl_blog_post');
	...
	public function getSearchablePages($arrPages, $intRoot=0, $blnIsSitemap=false)
	{
		...
		// Get all blog archives
		$objArchive = BlogArchiveModel::findByProtected('');
		...
		if ($objArchive !== null)
		{
			...
			{
				// Skip blog archives without target page
				if (!$objArchive->jumpTo)
				{
					continue;
				}

				// Skip blog archives outside the root nodes
				...
				// Get the items
				$objArticle = BlogPostModel::findPublishedDefaultByPid($objArchive->id);
	 ...
	 *
	 * @param BlogPostModel $objItem
	 * @param boolean   $blnAddArchive
	 * @param boolean   $blnAbsolute
	 *
	 * @return string
	 */
	public static function generateBlogUrl($objItem, $blnAddArchive=false, $blnAbsolute=false)
	{
		...
		if (self::$arrUrlCache[$strCacheKey] === null)
		{
			...
			// Add the current archive parameter (blog archive)
	...
	/**
	 * Return the schema.org data from a blog article
	 *
	 * @param BlogPostModel $objArticle
	 *
	 * @return array
	 */
	public static function getSchemaOrgData(BlogPostModel $objArticle): array
	{
		$htmlDecoder = System::getContainer()->get('contao.string.html_decoder');

		$jsonLd = array(
			'@type' => 'BlogArticle',
			'identifier' => '#/schema/blog/' . $objArticle->id,
			'url' => self::generateBlogUrl($objArticle),
	...
	/**
	 * Return the link of a blog article
	 *
	 * @param BlogPostModel $objItem
	 * @param string    $strUrl
	...
	{
		$arrFeeds = array();
		$objFeeds = BlogFeedModel::findAll();

		if ($objFeeds !== null)
		{
			while ($objFeeds->next())
			{
				$arrFeeds[] = $objFeeds->alias ?: 'blog' . $objFeeds->id;
			}
		}
